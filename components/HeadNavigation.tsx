...
export default function Header() {
  ...
  return (
    <header className={`absolute top-0 left-0 w-full bg-white shadow-md z-50 ${
      ...
      <div className="container mx-auto px-3 flex justify-between items-center py-0">
        {/* Logo *
        <a href="#" className="logo">
          <img 
            src="/assets/images/logo.png" 
            alt="Enlightened Future" 
            className="w-20 h-22 dark-logo" 
          />
        </a>
        ...
        <nav className="hidden lg:flex flex-grow items-center justify-between ml-8">
          <ul className="flex gap-8 mx-auto">
            {navLinks.map((link) => (
              <li key={link}>
                <a href="#" className="text-blue-900 hover:text- [#d20ac3] transition-colors font-bold">
                  {link}
                </a>
              </li>
            ))}
          ...
          <div className="flex items-center gap-6">
            <div className="flex gap-4">
              <a href="mailto:info@email.com" className="text-gray-600 hover:text- [#d20ac3]">
                info@email.com
              </a>
              <a href="tel:001234567890" className="text-gray-600 hover:text- [#d20ac3]">
                00 (123) 456 78 90
              </a>
            </div>
            ...
            <ul className="flex gap-4">
              {socialIcons.map(({ Icon, name }) => (
                <li key={name}>
                  <a href="#" className="text-blue-900 hover:text- [#d20ac3]">
                    <Icon size={20} />
                  </a>
                </li>
              ))}
        ...
        >
          ...
          <ul className="space-y-4 mb-8">
            {navLinks.map((link) => (
              <li key={link}>
                <a 
                  href="#" 
                  className="block py-2 font-bold hover:text- [#d20ac3] transition-colors"
                  onClick={closeMenu}
                >
                  {link}
                </a>
          ...
          <div className="mb-8 space-y-2">
            <a href="mailto:info@email.com" className="block hover:text- [#d20ac3] transition-colors">
              info@email.com
            </a>
            <a href="tel:001234567890" className="block hover:text- [#d20ac3] transition-colors">
              00 (123) 456 78 90
            </a>
          </div>
